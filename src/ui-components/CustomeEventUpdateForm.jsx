...
export default function CustomeEventUpdateForm(props) {
  const {
    id: idProp,
    customeEvent: customeEventModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = customeEventRecord
      ? { ...initialValues, ...customeEventRecord }
      : initialValues;
  ...
  const [customeEventRecord, setCustomeEventRecord] = React.useState(
    customeEventModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(CustomeEvent, idProp)
        : customeEventModelProp;
      setCustomeEventRecord(record);
    };
    queryData();
  }, [idProp, customeEventModelProp]);
  React.useEffect(resetStateValues, [customeEventRecord]);
  const validations = {
    EventName: [],
    EventDate: [],
    EventTime: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            CustomeEvent.copyOf(customeEventRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "CustomeEventUpdateForm")}
      {...rest}
    >
      <TextField
        label="Event name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || customeEventModelProp)}
