...
export default function ConfectionaryUpdateForm(props) {
  const {
    id: idProp,
    confectionary: confectionaryModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = confectionaryRecord
      ? { ...initialValues, ...confectionaryRecord }
      : initialValues;
    setMuffin(cleanValues.Muffin);
    setCakeSlice(cleanValues.CakeSlice);
    setCookies(cleanValues.Cookies);
    setButtons(cleanValues.Buttons);
    setQuavers(cleanValues.Quavers);
    setPombears(cleanValues.Pombears);
    setJazzles(cleanValues.Jazzles);
    setPringles(cleanValues.Pringles);
    setRaisins(cleanValues.Raisins);
    setSweetCone(cleanValues.SweetCone);
    setCrisps60p(cleanValues.Crisps60p);
    setCrisps35p(cleanValues.Crisps35p);
    setErrors({});
  };
  const [confectionaryRecord, setConfectionaryRecord] = React.useState(
    confectionaryModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Confectionary, idProp)
        : confectionaryModelProp;
      setConfectionaryRecord(record);
    };
    queryData();
  }, [idProp, confectionaryModelProp]);
  React.useEffect(resetStateValues, [confectionaryRecord]);
  const validations = {
    Muffin: [],
    CakeSlice: [],
    Cookies: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Confectionary.copyOf(confectionaryRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "ConfectionaryUpdateForm")}
      {...rest}
    >
      <TextField
        label="Muffin"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || confectionaryModelProp)}
