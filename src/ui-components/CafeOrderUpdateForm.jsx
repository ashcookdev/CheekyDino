...
export default function CafeOrderUpdateForm(props) {
  const {
    id: idProp,
    cafeOrder: cafeOrderModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const [SessionEmail, setSessionEmail] = React.useState(
    initialValues.SessionEmail
  );
  const [errors, setErrors] = React.useState({});
  const resetStateValues = () => {
    const cleanValues = cafeOrderRecord
      ? { ...initialValues, ...cafeOrderRecord }
      : initialValues;
    ...
    setSessionEmail(cleanValues.SessionEmail);
    setErrors({});
  };
  const [cafeOrderRecord, setCafeOrderRecord] =
    React.useState(cafeOrderModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(CafeOrder, idProp)
        : cafeOrderModelProp;
      setCafeOrderRecord(record);
    };
    queryData();
  }, [idProp, cafeOrderModelProp]);
  React.useEffect(resetStateValues, [cafeOrderRecord]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            CafeOrder.copyOf(cafeOrderRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "CafeOrderUpdateForm")}
      {...rest}
    >
      <TextField
        label="Created time"
      ...
      <TextField
        ...
        onChange={(e) => {
          ...
          if (onChange) {
            ...
            const result = onChange(modelFields);
            value = result?.SessionEmail ?? value;
          }
          if (errors.SessionEmail?.hasError) {
            runValidationTasks("SessionEmail", value);
          }
          setSessionEmail(value);
        }}
        onBlur={() => runValidationTasks("SessionEmail", SessionEmail)}
        errorMessage={errors.SessionEmail?.errorMessage}
        hasError={errors.SessionEmail?.hasError}
        {...getOverrideProps(overrides, "SessionEmail")}
      ></TextField>
      <Flex
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || cafeOrderModelProp)}
