...
export default function Example({ reply }) {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      ...
      const filteredMessages = messages.filter(
        (message) => (message.From === userEmail && message.To === replys.From) || (message.From === replys.From && message.To === userEmail)
      );
  
      // If there are no messages matching the filter, set messages to replys.
      if (filteredMessages.length === 0) {
        setMessages(replys);
      } else {
        // Sort the messages by the 'createdAt' field in ascending order
  ...
  const handleSendReply = async () => {
    ...
    const findMessage = await DataStore.query(CustomerEnquiries);
    const filter = findMessage.filter(
      (message) => message.id === replys.id
    );

console.log(filter);

    const save = await DataStore.save(
      CustomerEnquiries.copyOf(filter[0], updated => {
        updated.Replied = true;
      })
    );
  ...
  {messages.map((message, index) => (
    <div key={index} className="space-y-2 px-4 sm:grid sm:grid-cols-3 sm:gap-4 sm:space-y-0 sm:px-6 sm:py-5">
      <div>
        <div className="max-h-96 overflow-y-auto">
          <div className="flex items-center space-x-2">
            <p className="text-sm font-medium text-purple-600">{message.From}</p>
          </div>
        </div>
        <p className="text-sm text-gray-500">{format(new Date(message.createdAt), 'dd/MM/yyyy HH:mm')}</p>
      </div>
