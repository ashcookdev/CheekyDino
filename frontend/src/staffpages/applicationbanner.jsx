  ...
  useEffect(() => {
    const getJobApplication = async () => {
      try {
        ...
        const filterApplications = jobApplications.filter(application => 
        ...
        // Check if there is an interview scheduled for today
        const today = new Date().toLocaleDateString();
        const todayInterview = filterApplications.find(application => application.Date === today);
        if (todayInterview) {
